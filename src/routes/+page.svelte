  ...
  {:else}
    ...
    <div class="flex-1 flex flex-col">
      ...
      <div class="mb-8 p-6 bg-[--color-rai-card] rounded-xl border border-[--color-rai-border]">
        ...
        {#if hasError && appState.last_error}
          <div class="mb-4 p-3 bg-red-500/10 border border-red-500/20 rounded-lg">
            <p class="text-sm text-red-400">{appState.last_error}</p>
          </div>
        {/if}
      ...
      {#if isConnected}
        <div class="grid grid-cols-2 gap-4 mb-8">
          <StatsCard
            ...
            value={appState.requests_proxied}
            icon="📡"
          />
          <StatsCard
            label="Beatmaps Downloaded"
            value={appState.beatmaps_downloaded}
            icon="🎵"
          />
        </div>
      {/if}
