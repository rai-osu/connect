...
use crate::domain::{map_host_to_ppy, route_request, AppState, RouteDecision};
...
/// Runs the HTTPS proxy server with TLS termination.
///
/// This function is similar to `run_http_proxy` but accepts HTTPS connections
/// using a self-signed certificate. This is required for osu! when using
/// `-devserver localhost` which expects HTTPS.
///
/// # Certificate
///
/// A self-signed certificate is generated at startup that is valid for:
/// - `localhost` and `127.0.0.1`
/// - `osu.ppy.sh` and various `*.ppy.sh` subdomains
///
/// **Note**: Users must trust this certificate in their system for osu! to
...
///
/// Returns `Ok(())` when the server shuts down gracefully, or an error if
/// binding to the port fails or TLS setup fails.
            ...
            result = listener.accept() => {
                ...
                tokio::spawn(async move {
                    ...
                    if let Err(err) = http1::Builder::new()
                        ...
                        .await
                    {
                        tracing::error!("HTTPS connection error: {:?}", err);
                    }
                });
            }
            _ = &mut shutdown => {
...
///
/// This is the core forwarding function used by both `forward_to_raimoe` and
/// `forward_to_ppy`. It handles:
///
